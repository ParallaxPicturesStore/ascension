...
class VPNManagerModule: NSObject {
    ...
    @objc func storeCredentials(
        ...
        guard let defaults = UserDefaults(suiteName: "group.app.getascension") else {
            reject("CREDENTIALS_ERROR", "Could not access App Group container", nil)
            return
        }
        defaults.set(userId, forKey: "userId")
        defaults.set(supabaseUrl, forKey: "supabaseUrl")
    ...
    /// Get the total number of blocked attempts directly from the App Group.
    @objc func getBlockedCount(
        _ resolve: @escaping RCTPromiseResolveBlock,
        rejecter reject: @escaping RCTPromiseRejectBlock
    ) {
        let defaults = UserDefaults(suiteName: "group.app.getascension")
        let count = defaults?.integer(forKey: "blockedCount") ?? 0
        resolve(count)
    }
    ...
    @objc func getRecentBlocked(
        ...
        let defaults = UserDefaults(suiteName: "group.app.getascension")
        let all = defaults?.array(forKey: "blockedAttempts") as? [[String: Any]] ?? []
        let unreported = all.filter { ($0["reportedToServer"] as? Bool) != true }
        let mapped = unreported.map { entry -> [String: Any] in
            return [
